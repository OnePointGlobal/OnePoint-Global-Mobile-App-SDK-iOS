...
#import "OPG.h"
#import "OPGFile.h"
#import "OPGLocalFilesystem.h"
#import "OPGAssetLibraryFilesystem.h"

OPGFile *filePlugin = nil;
...
//NSString* const kOPGFilesystemURLPrefix = @"cdvfile";

@implementation OPGFilesystemURL
@synthesize url=_url;
...
{
    if ([[uri scheme] isEqualToString:@"cdvfile"] && [[uri host] isEqualToString:@"localhost"]) {
...
- (NSString *)fullPathForLocalURI:(NSURL *)uri
{
    if ([[uri scheme] isEqualToString:@"cdvfile"] && [[uri host] isEqualToString:@"localhost"]) {
        ...
        return [[uri absoluteString] substringFromIndex:[@"assets-library" length]+2];
    }
    return nil;
}
...
{
    return [[OPGFilesystemURL alloc] initWithString:strURL];
}

+ (OPGFilesystemURL *)fileSystemURLWithURL:(NSURL *)URL
{
    return [[OPGFilesystemURL alloc] initWithURL:URL];
}
...
@implementation OPGFilesystemURLProtocol

+ (BOOL)canInitWithRequest:(NSURLRequest*)request
{
    NSURL* url = [request URL];
    return [[url scheme] isEqualToString:@"cdvfile"];
}
...
- (void)startLoading
{
    OPGFilesystemURL* url = [OPGFilesystemURL fileSystemURLWithURL:[[self request] URL]];
    NSObject<OPGFileSystem> *fs = [filePlugin filesystemForURL:url];
...
- (NSObject<OPGFileSystem> *)fileSystemByName:(NSString *)fsName
{
    if (self.fileSystems != nil) {
        for (NSObject<OPGFileSystem> *fs in self.fileSystems) {
...
- (NSArray *)getExtraFileSystemsPreference:(UIViewController *)vc
{
    ...
    if([self.viewController isKindOfClass:[OPGBaseController class]]) {
        OPGBaseController *vc = (OPGBaseController *)self.viewController;
...
- (void)registerExtraFileSystems:(NSArray *)filesystems fromAvailableSet:(NSDictionary *)availableFileSystems
{
    ...
    for (NSString *fsName in filesystems) {
        if (![installedFilesystems containsObject:fsName]) {
            ...
            if (fsRoot) {
                [filePlugin registerFilesystem:[[OPGLocalFilesystem alloc] initWithName:fsName root:fsRoot]];
...
- (void)pluginInitialize
{
    ...
    if (location == nil) {
        ...
        // if we're not embedded in a OPGBaseController somehow.)
    ...
    if ([[NSFileManager defaultManager] createDirectoryAtPath:self.appTempPath
                                  withIntermediateDirectories:YES
                                                   attributes:nil
                                                        error:&error]) {
        [self registerFilesystem:[[OPGLocalFilesystem alloc] initWithName:@"temporary" root:self.appTempPath]];
    } else {
    ...
    if ([location isEqualToString:@"library"]) {
        if ([[NSFileManager defaultManager] createDirectoryAtPath:self.appLibraryPath
                                      withIntermediateDirectories:YES
                                                       attributes:nil
                                                            error:&error]) {
            [self registerFilesystem:[[OPGLocalFilesystem alloc] initWithName:@"persistent" root:self.appLibraryPath]];
        } else {
         ...
         *  versions.
         */
        [self registerFilesystem:[[OPGLocalFilesystem alloc] initWithName:@"persistent" root:self.rootDocsPath]];
    } else {
        NSAssert(false,
    ...
    [self registerFilesystem:[[OPGAssetLibraryFilesystem alloc] initWithName:@"assets-library"]];
...
{
    self = (OPGFile*)[super initWithWebView:theWebView];
    if (self) {
        filePlugin = self;
        [NSURLProtocol registerClass:[OPGFilesystemURLProtocol class]];
...
- (OPGFilesystemURL *)fileSystemURLforArg:(NSString *)urlArg
{
    OPGFilesystemURL* ret = nil;
    if ([urlArg hasPrefix:@"file://"]) {
        ...
        ret = [OPGFilesystemURL fileSystemURLWithString:urlArg];
    }
    return ret;
}
...
- (OPGFilesystemURL *)fileSystemURLforLocalPath:(NSString *)localPath
{
    ...
    for (id object in self.fileSystems) {
        if ([object respondsToSelector:@selector(URLforFilesystemPath:)]) {
            OPGFilesystemURL *url = [object URLforFilesystemPath:localPath];
            if (url){
...
- (void)requestFileSystem:(OPGInvokedUrlCommand*)command
{
    ...
    if (type > self.fileSystems.count) {
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsInt:NOT_FOUND_ERR];
        ...
        if (pNumAvail && ([pNumAvail unsignedLongLongValue] < size)) {
            result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:QUOTA_EXCEEDED_ERR];
        } else {
            NSObject<OPGFileSystem> *rootFs = [self.fileSystems objectAtIndex:type];
            if (rootFs == nil) {
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsInt:NOT_FOUND_ERR];
                NSLog(@"No filesystem of type requested");
            } else {
                ...
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:fileSystem];
            }
        }
    }
...
- (void)requestAllFileSystems:(OPGInvokedUrlCommand*)command
{
    NSMutableArray* ret = [[NSMutableArray alloc] init];
    for (NSObject<OPGFileSystem>* root in fileSystems_) {
        [ret addObject:[self makeEntryForPath:@"/" fileSystemName:root.name isDirectory:YES]];
    }
    OPGPluginResult* result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArray:ret];
    [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
}

- (void)requestAllPaths:(OPGInvokedUrlCommand*)command
{
    ...
    OPGPluginResult* result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:ret];
    [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
}
...
{
    NSObject<OPGFileSystem> *fs = [self fileSystemByName:fsName];
    return [fs makeEntryForPath:fullPath isDirectory:isDir];
}

- (NSDictionary *)makeEntryForLocalURL:(OPGFilesystemURL *)localURL
{
    NSObject<OPGFileSystem> *fs = [self filesystemForURL:localURL];
    return [fs makeEntryForLocalURL:localURL];
}

- (NSDictionary *)makeEntryForURL:(NSURL *)URL
{
    OPGFilesystemURL* fsURL = [self fileSystemURLforArg:[URL absoluteString]];
...
- (void)resolveLocalFileSystemURI:(OPGInvokedUrlCommand*)command
{
    ...
    OPGFilesystemURL* inputURI = [self fileSystemURLforArg:localURIstr];
    
    if (inputURI == nil || inputURI.fileSystemName == nil) {
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsInt:ENCODING_ERR];
    } else {
        NSObject<OPGFileSystem> *fs = [self filesystemForURL:inputURI];
        if (fs == nil) {
            result = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsInt:ENCODING_ERR];
        } else {
...
- (void)getDirectory:(OPGInvokedUrlCommand*)command
{
    ...
    OPGInvokedUrlCommand* subCommand =
        [[OPGInvokedUrlCommand alloc] initWithArguments:arguments
...
- (void)getFile:(OPGInvokedUrlCommand*)command
{
    ...
    OPGFilesystemURL* baseURI = [self fileSystemURLforArg:baseURIstr];
    ...
    NSObject<OPGFileSystem> *fs = [self filesystemForURL:baseURI];
    OPGPluginResult* result = [fs getFileForURL:baseURI requestedPath:requestedPath options:options];
...
- (void)getParent:(OPGInvokedUrlCommand*)command
{
    ...
    OPGFilesystemURL* localURI = [self fileSystemURLforArg:command.arguments[0]];

    NSObject<OPGFileSystem> *fs = [self filesystemForURL:localURI];
    OPGPluginResult* result = [fs getParentForURL:localURI];
 ...
 */
- (void)setMetadata:(OPGInvokedUrlCommand*)command
{
    // arguments
    OPGFilesystemURL* localURI = [self fileSystemURLforArg:command.arguments[0]];
    NSDictionary* options = [command.arguments objectAtIndex:1 withDefault:nil];
    NSObject<OPGFileSystem> *fs = [self filesystemForURL:localURI];
    OPGPluginResult* result = [fs setMetadataForURL:localURI withObject:options];
...
- (void)remove:(OPGInvokedUrlCommand*)command
{
    ...
    if ([localURI.fullPath isEqualToString:@""]) {
        ...
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:NO_MODIFICATION_ALLOWED_ERR];
    } else {
        NSObject<OPGFileSystem> *fs = [self filesystemForURL:localURI];
...
- (void)doCopyMove:(OPGInvokedUrlCommand*)command isCopy:(BOOL)bCopy
{
    ...
    if (!srcURLstr || !destURLstr) {
        ...
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:NOT_FOUND_ERR];
        [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
        return;
    }

    OPGFilesystemURL* srcURL = [self fileSystemURLforArg:srcURLstr];
    OPGFilesystemURL* destURL = [self fileSystemURLforArg:destURLstr];

    NSObject<OPGFileSystem> *srcFs = [self filesystemForURL:srcURL];
    NSObject<OPGFileSystem> *destFs = [self filesystemForURL:destURL];
    ...
    if ([newName rangeOfString:@":"].location != NSNotFound) {
        ...
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:ENCODING_ERR];
        [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
        return;
    }

    [destFs copyFileToURL:destURL withName:newName fromFileSystem:srcFs atURL:srcURL copy:bCopy callback:^(OPGPluginResult* result) {
        [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
    }];
...
- (void)getFileMetadata:(OPGInvokedUrlCommand*)command
{
    ...
    [fs getFileMetadataForURL:localURI callback:^(OPGPluginResult* result) {
        [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
    }];
}
...
- (void)readEntries:(OPGInvokedUrlCommand*)command
{
    OPGFilesystemURL* localURI = [self fileSystemURLforArg:command.arguments[0]];
    NSObject<OPGFileSystem> *fs = [self filesystemForURL:localURI];
    OPGPluginResult *result = [fs readEntriesAtURL:localURI];
...
- (void)readAsText:(OPGInvokedUrlCommand*)command
{
    ...
    if (fs == nil) {
        OPGPluginResult* result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:NOT_FOUND_ERR];
        [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
        return;
    }
    ...
    if ([@"UTF-8" caseInsensitiveCompare : encoding] != NSOrderedSame) {
        ...
        OPGPluginResult* result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:ENCODING_ERR];
        [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
        return;
    }
    ...
    [self.commandDelegate runInBackground:^ {
        [fs readFileAtURL:localURI start:start end:end callback:^(NSData* data, NSString* mimeType, CDVFileError errorCode) {
            ...
            if (data != nil) {
                ...
                if (str != nil) {
                    result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:str];
            ...
            if (result == nil) {
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:errorCode];
            }
    ...
    [self.commandDelegate runInBackground:^ {
        [fs readFileAtURL:localURI start:start end:end callback:^(NSData* data, NSString* mimeType, CDVFileError errorCode) {
            ...
            if (data != nil) {
                ...
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:output];
            } else {
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:errorCode];
            }
...
- (void)readAsArrayBuffer:(OPGInvokedUrlCommand*)command
{
    ...
    [self.commandDelegate runInBackground:^ {
        [fs readFileAtURL:localURI start:start end:end callback:^(NSData* data, NSString* mimeType, CDVFileError errorCode) {
            OPGPluginResult* result = nil;
            if (data != nil) {
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArrayBuffer:data];
            } else {
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:errorCode];
            }
...
- (void)readAsBinaryString:(OPGInvokedUrlCommand*)command
{
    OPGFilesystemURL* localURI = [self fileSystemURLforArg:command.arguments[0]];
    NSInteger start = [[command argumentAtIndex:1] integerValue];
    NSInteger end = [[command argumentAtIndex:2] integerValue];
    ...
    [self.commandDelegate runInBackground:^ {
        [fs readFileAtURL:localURI start:start end:end callback:^(NSData* data, NSString* mimeType, CDVFileError errorCode) {
            ...
            if (data != nil) {
                ...
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:payload];
                result.associatedObject = data;
            } else {
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:errorCode];
            }
...
- (void)truncate:(OPGInvokedUrlCommand*)command
{
    ...
    NSObject<OPGFileSystem> *fs = [self filesystemForURL:localURI];
    OPGPluginResult *result = [fs truncateFileAtURL:localURI atPosition:pos];
...
- (void)write:(OPGInvokedUrlCommand*)command
{
    ...
    if ([argData isKindOfClass:[NSString class]]) {
        ...
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"Invalid parameter type"];
    }
...
- (NSString *)filesystemPathForURL:(OPGFilesystemURL *)localURL
{
    for (NSObject<OPGFileSystem> *fs in self.fileSystems) {
...
- (void)testFileExists:(OPGInvokedUrlCommand*)command
{
    ...
    OPGPluginResult* result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsInt:(bExists ? 1 : 0)];
...
- (void)testDirectoryExists:(OPGInvokedUrlCommand*)command
{
    ...
    OPGPluginResult* result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsInt:((bExists && bIsDir) ? 1 : 0)];
...
- (void)getFreeDiskSpace:(OPGInvokedUrlCommand*)command
{
    ...
    OPGPluginResult* result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:strFreeSpace];
...
{
    return [OPGLocalFilesystem getMimeTypeFromPath:fullPath];
}

- (NSDictionary *)getDirectoryEntry:(NSString *)localPath isDirectory:(BOOL)bDirRequest
{
    OPGFilesystemURL *localURL = [self fileSystemURLforLocalPath:localPath];
...
{
    OPGFilesystemURL* localURL = [self fileSystemURLforArg:command.arguments[0]];

    NSString* fsPath = [self filesystemPathForURL:localURL];
    OPGPluginResult* result;
    if (fsPath) {
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:fsPath];
    } else {
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"Cannot resolve URL to a file"];
    }
    [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
}
