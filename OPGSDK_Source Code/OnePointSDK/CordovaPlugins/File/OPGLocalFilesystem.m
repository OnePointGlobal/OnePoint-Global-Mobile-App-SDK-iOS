...
@implementation OPGLocalFilesystem
 ...
 *  NSString localURI
 * OUT
 *  OPGPluginResult result containing a file or directoryEntry for the localURI, or an error if the
...
- (OPGPluginResult *)entryForLocalURI:(OPGFilesystemURL *)url
{
    ...
    if (entry) {
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:entry];
    } else {
...
- (OPGFilesystemURL *)URLforFullPath:(NSString *)fullPath
{
    if (fullPath) {
        NSString* escapedPath = [fullPath stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        if ([fullPath hasPrefix:@"/"]) {
            return [OPGFilesystemURL fileSystemURLWithString:[NSString stringWithFormat:@"%@://localhost/%@%@", @"cdvfile", self.name, escapedPath]];
        }
        return [OPGFilesystemURL fileSystemURLWithString:[NSString stringWithFormat:@"%@://localhost/%@/%@", @"cdvfile", self.name, escapedPath]];
    }
    return nil;
...
- (OPGPluginResult *)getFileForURL:(OPGFilesystemURL *)baseURI requestedPath:(NSString *)requestedPath options:(NSDictionary *)options
{
    ...
    if ([requestedPath rangeOfString:@":"].location != NSNotFound) {
        ...
        OPGFilesystemURL* requestedURL = [self URLforFullPath:combinedPath];
        ...
        if (bExists && (create == NO) && (bIsDir == !bDirRequest)) {
            ...
            if (!bSuccess) {
                ...
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:[self makeEntryForPath:requestedURL.fullPath isDirectory:bDirRequest]];
            }
...
{
    OPGPluginResult* result = nil;
    OPGFilesystemURL *newURI = nil;
    if ([localURI.fullPath isEqualToString:@""]) {
        // return self
        newURI = localURI;
    } else {
        newURI = [OPGFilesystemURL fileSystemURLWithURL:[localURI.url URLByDeletingLastPathComponent]]; /* TODO: UGLY - FIX */
    }
    NSFileManager* fileMgr = [[NSFileManager alloc] init];
    BOOL bIsDir;
    BOOL bExists = [fileMgr fileExistsAtPath:[self filesystemPathForURL:newURI] isDirectory:&bIsDir];
    if (bExists) {
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:[self makeEntryForPath:newURI.fullPath isDirectory:bIsDir]];
    } else {
...
- (OPGPluginResult*)setMetadataForURL:(OPGFilesystemURL *)localURI withObject:(NSDictionary *)options
{
    ...
    if (ok) {
        return [OPGPluginResult resultWithStatus:CDVCommandStatus_OK];
    } else {
...
- (OPGPluginResult*)doRemove:(NSString*)fullPath
{
    OPGPluginResult* result = nil;
    BOOL bSuccess = NO;
    NSError* __autoreleasing pError = nil;
    ...
    @try {
        ...
        if (bSuccess) {
            result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK];
        } else {
        ...
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsInt:SYNTAX_ERR];
    }
...
- (OPGPluginResult *)removeFileAtURL:(OPGFilesystemURL *)localURI
{
    ...
    if (!bExists) {
        return [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:NOT_FOUND_ERR];
    }
...
- (OPGPluginResult *)readEntriesAtURL:(OPGFilesystemURL *)localURI
{
    ...
    if (contents) {
        ...
        return [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArray:entries];
    } else {
...
- (OPGPluginResult *)truncateFileAtURL:(OPGFilesystemURL *)localURI atPosition:(unsigned long long)pos
{
    ...
    return [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsInt:(int)newPos];
}
...
- (OPGPluginResult *)writeToFileAtURL:(OPGFilesystemURL *)localURL withData:(NSData*)encData append:(BOOL)shouldAppend
{
    ...
    if (filePath) {
        ...
        if (fileStream) {
            ...
            if (len == 0) {
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsInt:len];
            } else {
                [fileStream open];
                ...
                [fileStream close];
                if (bytesWritten > 0) {
                    result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsInt:bytesWritten];
                    // } else {
    ...
    if (!result) {
        // was an error
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsInt:errCode];
    }
    return result;
}
...
- (void)copyFileToURL:(OPGFilesystemURL *)destURL withName:(NSString *)newName fromFileSystem:(NSObject<OPGFileSystem> *)srcFs atURL:(OPGFilesystemURL *)srcURL copy:(BOOL)bCopy callback:(void (^)(OPGPluginResult *))callback
{
    ...
    if (!bDestExists) {
    ...
    else if ([srcFs isKindOfClass:[OPGLocalFilesystem class]]) {
        ...
        if (!bSrcExists) {
            ...
            if (bSuccess) {
                ...
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:newEntry];
            } else {
                if (error) {
        ...
        [srcFs readFileAtURL:srcURL start:0 end:-1 callback:^(NSData* data, NSString* mimeType, CDVFileError errorCode) {
            ...
            if (data != nil) {
                ...
                if (bSuccess) {
                    ...
                    result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:ABORT_ERR];
                }
            } else {
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:errorCode];
            }
    ...
    if (result == nil) {
        ...
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsInt:errCode];
    }
    callback(result);
}
...
- (void)readFileAtURL:(OPGFilesystemURL *)localURL start:(NSInteger)start end:(NSInteger)end callback:(void (^)(NSData*, NSString* mimeType, CDVFileError))callback
{
    ...
    NSString* mimeType = [OPGLocalFilesystem getMimeTypeFromPath:path];
    if (mimeType == nil) {
        mimeType = @"*/*";
    }
...
- (void)getFileMetadataForURL:(OPGFilesystemURL *)localURL callback:(void (^)(OPGPluginResult *))callback
{
    ...
    if (fileAttrs) {
        ...
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:fileInfo];
        ...
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsInt:errorCode];
    }
