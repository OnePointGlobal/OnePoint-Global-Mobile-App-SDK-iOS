...
@implementation OPGPictureOptions

+ (instancetype) createFromTakePictureArguments:(OPGInvokedUrlCommand*)command
{
    OPGPictureOptions* pictureOptions = [[OPGPictureOptions alloc] init];
...
- (void)takePicture:(OPGInvokedUrlCommand*)command
{
    ...
    __weak OPGCamera* weakSelf = self;

    [self.commandDelegate runInBackground:^{
        
        OPGPictureOptions* pictureOptions = [OPGPictureOptions createFromTakePictureArguments:command];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    OPGPluginResult* result = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"has no access to camera"];   // error callback expects string ATM
...
- (void)cleanup:(OPGInvokedUrlCommand*)command
{
    ...
    if (hasErrors) {
        pluginResult = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsString:@"One or more files failed to be deleted."];
    } else {
...
- (NSData*)processImage:(UIImage*)image info:(NSDictionary*)info options:(OPGPictureOptions*)options
{
    NSData* data = nil;
...
- (UIImage*)retrieveImage:(NSDictionary*)info options:(OPGPictureOptions*)options
{
    // get the image
    UIImage* image = nil;
...
- (OPGPluginResult*)resultForImage:(OPGPictureOptions*)options info:(NSDictionary*)info
{
    ...
    switch (options.destinationType) {
        case DestinationTypeNativeUri:
        {
            ...
            result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:nativeUri];
            saveToPhotoAlbum = NO;
        }
            break;
        ...
        case DestinationTypeDataUrl:
        {
            ...
            if (data)  {
                result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:toBase64(data)];
            }
        }
            break;
        default:
...
- (OPGPluginResult*)resultForVideo:(NSDictionary*)info
{
    NSString* moviePath = [[info objectForKey:UIImagePickerControllerMediaURL] absoluteString];
    return [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:moviePath];
}
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary*)info
{
    ...
    dispatch_block_t invoke = ^(void) {
        __block OPGPluginResult* result = nil;
...
{
    OPGPictureOptions* options = self.pickerController.pictureOptions;
    OPGPluginResult* result = nil;
...
+ (instancetype) createFromPictureOptions:(OPGPictureOptions*)pictureOptions;
{
