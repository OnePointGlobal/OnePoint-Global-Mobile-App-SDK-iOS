...
- (void)captureAudio:(OPGInvokedUrlCommand*)command
{
    ...
    if (NSClassFromString(@"AVAudioRecorder") == nil) {
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageToErrorObject:CAPTURE_NOT_SUPPORTED];
    } else if (self.inUse == YES) {
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageToErrorObject:CAPTURE_APPLICATION_BUSY];
    } else {
...
//    }
    ...
    if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        if (pickerController == nil) {
            pickerController = [[OPGImagePicker alloc] init];
        }
...
- (OPGPluginResult*)processImage:(UIImage*)image type:(NSString*)mimeType forCallbackId:(NSString*)callbackId
{
    ...
    if (![data writeToFile:filePath options:NSAtomicWrite error:&err]) {
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageToErrorObject:CAPTURE_INTERNAL_ERR];
        if (err) {
        ...
        result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArray:fileArray];
    }
...
- (OPGPluginResult*)processVideo:(NSString*)moviePath forCallbackId:(NSString*)callbackId
{
    ...
    return [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArray:fileArray];
}
...
- (void)getFormatData:(OPGInvokedUrlCommand*)command
{
    ...
    BOOL bError = NO;
    OPGCaptureError errorCode = CAPTURE_INTERNAL_ERR;
    OPGPluginResult* result = nil;
...
{
    OPGImagePicker* cameraPicker = (OPGImagePicker*)picker;
...
- (id)initWithCommand:(OPGCapture*)theCommand duration:(NSNumber*)theDuration callbackId:(NSString*)theCallbackId
{
    if ((self = [super init])) {
...
- (void)dismissAudioView:(id)sender
{
    ...
    if (!self.pluginResult) {
        // return error
        self.pluginResult = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageToErrorObject:(int)self.errorCode];
    }
...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder*)recorder successfully:(BOOL)flag
{
    ...
    if (flag) {
        ...
        self.pluginResult = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArray:fileArray];
    } else {
        self.pluginResult = [OPGPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageToErrorObject:CAPTURE_INTERNAL_ERR];
    }
}
