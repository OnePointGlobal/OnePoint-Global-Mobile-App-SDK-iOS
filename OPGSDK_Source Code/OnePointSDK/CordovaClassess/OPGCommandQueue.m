...
@implementation OPGCommandQueue
...
- (void)enqueueCommandBatch:(NSString*)batchJSON
{
    if ([batchJSON length] > 0) {
        ...
        if ([batchJSON length] < JSON_SIZE_FOR_MAIN_THREAD) {
            [commandBatchHolder addObject:[batchJSON opg_JSONObject]];
        } else {
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^() {
                    NSMutableArray* result = [batchJSON opg_JSONObject];
        ...
        while ([_queue count] > 0) {
            ...
            while ([commandBatch count] > 0) {
                @autoreleasepool {
                    ...
                    OPGInvokedUrlCommand* command = [OPGInvokedUrlCommand commandFromJson:jsonEntry];
                    CDV_EXEC_LOG(@"Exec(%@): Calling %@.%@", command.callbackId, command.className, command.methodName);

                    if (![self execute:command]) {
#ifdef DEBUG
                            NSString* commandJson = [jsonEntry opg_JSONString];
...
- (BOOL)execute:(OPGInvokedUrlCommand*)command
{
    ...
    OPGPlugin* obj = [_viewController.commandDelegate getCommandInstance:command.className];

    if (!([obj isKindOfClass:[OPGPlugin class]])) {
        NSLog(@"ERROR: Plugin '%@' not found, or is not a OPGPlugin. Check your plugin mapping in config.xml.", command.className);
        return NO;
    }
    BOOL retVal = YES;
