...
@interface OPGWhitelistPattern : NSObject {
    @private
...
@implementation OPGWhitelistPattern
...
- (id)initWithScheme:(NSString*)scheme host:(NSString*)host port:(NSString*)port path:(NSString*)path
{
    ...
    if (self) {
        if ((scheme == nil) || [scheme isEqualToString:@"*"]) {
            ...
            _scheme = [NSRegularExpression regularExpressionWithPattern:[OPGWhitelistPattern regexFromPattern:scheme allowWildcards:NO] options:NSRegularExpressionCaseInsensitive error:nil];
        }
        if ([host isEqualToString:@"*"]) {
            _host = nil;
        } else if ([host hasPrefix:@"*."]) {
            _host = [NSRegularExpression regularExpressionWithPattern:[NSString stringWithFormat:@"([a-z0-9.-]*\\.)?%@", [OPGWhitelistPattern regexFromPattern:[host substringFromIndex:2] allowWildcards:false]] options:NSRegularExpressionCaseInsensitive error:nil];
        } else {
            _host = [NSRegularExpression regularExpressionWithPattern:[OPGWhitelistPattern regexFromPattern:host allowWildcards:NO] options:NSRegularExpressionCaseInsensitive error:nil];
        }
        ...
        if ((path == nil) || [path isEqualToString:@"/*"]) {
            ...
            _path = [NSRegularExpression regularExpressionWithPattern:[OPGWhitelistPattern regexFromPattern:path allowWildcards:YES] options:0 error:nil];
        }
    }
    return self;
}
       ...
       // NSLog(@"Unlimited access to network resources");
        ...
        if (m != nil) {
            ...
            if (scheme == nil) {
                ...
                [self.whitelist addObject:[[OPGWhitelistPattern alloc] initWithScheme:@"http" host:host port:port path:path]];
                [self.whitelist addObject:[[OPGWhitelistPattern alloc] initWithScheme:@"https" host:host port:port path:path]];
            } else {
                [self.whitelist addObject:[[OPGWhitelistPattern alloc] initWithScheme:scheme host:host port:port path:path]];
            }
...
- (BOOL)URLIsAllowed:(NSURL*)url logFailure:(BOOL)logFailure
{
    ...
    for (OPGWhitelistPattern* p in self.whitelist) {
        if ([p matches:url]) {
            return YES;
        }
    }
