...
#import "OPGConnection.h"
#import "OPGNReachability.h"

@interface OPGConnection (PrivateMethods)
- (void)updateOnlineStatus;
- (void)sendPluginResult;
@end

@implementation OPGConnection

@synthesize connectionType, internetReach;

- (void)getConnectionInfo:(OPGInvokedUrlCommand*)command
{
...
- (void)sendPluginResult
{
    OPGPluginResult* result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:self.connectionType];

    [result setKeepCallbackAsBool:YES];
    [self.commandDelegate sendPluginResult:result callbackId:_callbackId];
}

- (NSString*)w3cConnectionTypeFor:(OPGNReachability*)reachability
{
...
- (void)updateReachability:(OPGNReachability*)reachability
{
    if (reachability) {
...
{
    OPGNReachability* curReach = [note object];

    if ((curReach != nil) && [curReach isKindOfClass:[OPGNReachability class]]) {
...
- (OPGPlugin*)initWithWebView:(UIWebView*)theWebView
{
    ...
    if (self) {
        ...
        self.internetReach = [OPGNReachability reachabilityForInternetConnection];
