...
@interface OPGLocalStorage ()

@property (nonatomic, readwrite, strong) NSMutableArray* backupInfo;  // array of OPGBackupInfo objects
@property (nonatomic, readwrite, weak) id <UIWebViewDelegate> webviewDelegate;

@end

@implementation OPGLocalStorage
...
+ (NSMutableArray*)createBackupInfoWithTargetDir:(NSString*)targetDir backupDir:(NSString*)backupDir targetDirNests:(BOOL)targetDirNests backupDirNests:(BOOL)backupDirNests rename:(BOOL)rename
{
    ...
    backupItem = [[OPGBackupInfo alloc] init];
...
- (BOOL)shouldBackup
{
    for (OPGBackupInfo* info in self.backupInfo) {
        ...
        if ([info shouldBackup]) {
            ...
            if (callbackId) {
                if (error == nil) {
                    ...
                    result = [OPGPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:message];
                    [self.commandDelegate sendPluginResult:result callbackId:callbackId];
                } else {
                    message = [NSString stringWithFormat:@"Error in CDVLocalStorage (%@) backup: %@", info.label, [error localizedDescription]];
                    NSLog(@"%@", message);

                    result = [OPGPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:message];
                    [self.commandDelegate sendPluginResult:result callbackId:callbackId];
                }
            }
        }
...
+ (void)__restoreLegacyDatabaseLocationsWithBackupType:(NSString*)backupType
{
    ...
    for (OPGBackupInfo* info in backupInfo) {
...
- (void)onResignActive
{
    ...
    if (exitsOnSuspend) {
        ...
        OPGLocalStorage __weak* weakSelf = self;
...
#pragma mark -
#pragma mark OPGBackupInfo implementation

@implementation OPGBackupInfo
